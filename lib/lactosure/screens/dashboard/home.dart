...
class _DashboardhomeState extends State<Dashboardhome> {
  ...
  Future<void> writeCurrentCount(int count) async {
    try {
      // Build command
      ...
      debugPrint(
        "WRITE COUNT CMD => ${cmd.map((e) => e.toRadixString(16).padLeft(2, '0')).join(' ')}",
      );

      final response = await sendCommand(cmd);

      debugPrint(
        "WRITE COUNT RESPONSE => ${response.map((e) => e.toRadixString(16).padLeft(2, '0')).join(' ')}",
      );
    } catch (e) {
